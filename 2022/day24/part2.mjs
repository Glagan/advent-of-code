...
(async () => {
			...
			for (const neighbor of neighbors) {
				...
				if (gScore[neighborKey] === undefined || score < gScore[neighborKey]) {
					...
					if (openSet[neighborKey] === undefined) {
						...
						if (openSet.length > 100000) {
							openSet = openSet.slice(0, 100000);
						}
					}
				}
			}
	...
	// * Answer
	const firstTravel = aStar({ x: 1, y: 0, minute: 0 }, { x: xLength - 2, y: yLength - 1 });
	console.log("firstTravel", firstTravel);
	const returnTravel = aStar({ x: xLength - 2, y: yLength - 1, minute: firstTravel[0].minute }, { x: 1, y: 0 });
	console.log("returnTravel", returnTravel);
	const finalTravel = aStar({ x: 1, y: 0, minute: returnTravel[0].minute }, { x: xLength - 2, y: yLength - 1 });
	console.log("finalTravel", finalTravel);
	console.log("Answer:", finalTravel[0].minute);
})();

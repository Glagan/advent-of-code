...
(async () => {
	...
	const waitUntilEnd = (node) => {
		const remaining = 24 - node.minute;
		return {
			...node,
			minute: 24,
	...
	for (const blueprint of blueprints) {
		...
		while (openSet.length > 0) {
			...
			// * Create each robots
			let added = false;
			const timeRemaining = 24 - node.minute;
			// Geode
			if (node.obsidianRobots > 0) {
				...
				if (node.minute + timeUntilNextGeode < 24) {
					added = true;
					openSet.push({
						...node,
			...
			if (
				...
				if (node.minute + timeUntilNextObsidian < 24) {
					added = true;
					openSet.push({
						...node,
			...
			if (
				...
				if (node.minute + timeUntilNextClay < 24) {
					added = true;
					openSet.push({
						...node,
			...
			if (
				...
				if (node.minute + timeUntilNextOre < 24) {
					added = true;
					openSet.push({
						...node,
		...
		if (best) {
			answer += best.geode * blueprint.id;
	...
	// * Answer
	console.log("Answer:", answer, "expect", 33, "expect", 1356);
})();

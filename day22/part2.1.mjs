...
(async () => {
	...
	const squareMapping = {
		0: {
			[RIGHT]: { square: 1, direction: RIGHT },
			[DOWN]: { square: 2, direction: DOWN },
			[LEFT]: { square: 3, direction: RIGHT },
			[TOP]: { square: 5, direction: RIGHT },
		},
		1: {
			[RIGHT]: { square: 4, direction: LEFT },
			[DOWN]: { square: 2, direction: LEFT },
			[LEFT]: { square: 0, direction: LEFT },
			[TOP]: { square: 5, direction: TOP },
		},
		2: {
			[RIGHT]: { square: 1, direction: TOP },
			[DOWN]: { square: 4, direction: DOWN },
			[LEFT]: { square: 3, direction: DOWN },
			[TOP]: { square: 0, direction: TOP },
		},
		3: {
			[RIGHT]: { square: 4, direction: RIGHT },
			[DOWN]: { square: 5, direction: DOWN },
			[LEFT]: { square: 0, direction: RIGHT },
			[TOP]: { square: 2, direction: RIGHT },
		},
		4: {
			[RIGHT]: { square: 1, direction: LEFT },
			[DOWN]: { square: 5, direction: LEFT },
			[LEFT]: { square: 3, direction: LEFT },
			[TOP]: { square: 2, direction: TOP },
		},
		5: {
			[RIGHT]: { square: 4, direction: TOP },
			[DOWN]: { square: 1, direction: DOWN },
			[LEFT]: { square: 0, direction: DOWN },
	...
	const positionOnNewSquare = (squareStart, fromDirection, toDirection, x, y) => {
		if (fromDirection === TOP) {
			...
			if (toDirection === TOP) {
				return { x: squareStart.x + y, y: squareStart.y + (squareLength - 1) };
			} else if (toDirection === RIGHT) {
			...
			if (toDirection === TOP) {
				...
				return { x: squareStart.x, y: squareStart.y + (squareLength - 1 - y) };
			} else if (toDirection === LEFT) {
				return { x: squareStart.x + (squareLength - 1), y: squareStart.y + y };
			} else if (toDirection === DOWN) {
			...
			if (toDirection === TOP) {
				...
				return { x: squareStart.x + (squareLength - 1), y: squareStart.y + x };
			} else if (toDirection === DOWN) {
	...
	console.log("Answer:", 1000 * row + 4 * column + direction.value);
})();
